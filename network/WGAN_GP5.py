...
class WGAN_GP5(object):
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, test_dir, input_height,input_width,output_height,output_width,
        ...
        self.model_name = "WGAN-GP5"  # name for checkpoint
    ...
    def build_model(self):
        ...
        G = np.reshape(G, (self.batch_size*self.input_height, self.input_width))
        n_cross=0
        for i in range(G.shape[0]+1):
            if(np.sqrt(G[i][3]**2+G[i][4]**2)>120):
                n=n+1
        if(n_cross > 5):
            D_fake, D_fake_logits, _ = (self.discriminator(G, is_training=True, reuse=True))/10
        else:
    ...
    def create_time_file(self, result_dir,parameter_path, structure_path):
        data_now = time.strftime("%m%d-%H-%M", time.localtime())
    ...
    def output_csv(self, epoch):
        ...
        for i in range(epoch):
            ...
            #samples = (samples+1.)/2.
            samples = np.reshape(samples,(64, 3584))
            simul_data.extend(samples)
        simul_data = pd.DataFrame(simul_data)
        simul_data.to_csv('1.csv', index=False)
