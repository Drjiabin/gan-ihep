...
class WGAN_GP3(object):
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, test_dir, input_height,input_width,output_height,output_width,
        ...
        self.model_name = "WGAN-GP3"  # name for checkpoint
        ...
        if dataset_name == 'mnist' or dataset_name == 'comet':
            ...
            self.data_X = load_comet(self.dataset_name)
    ...
    def build_model(self):
        ...
        #对范围超出120的loss进行惩罚
        #G1 = tf.reshape(G, (self.batch_size*self.input_height, self.input_width))
        #n_cross = tf.constant(0)
        #loss_penalty = tf.constant(1)
        #add_penalty = tf.constant(1)
        #for i in range(self.input_height*self.batch_size):
        #    #condition_bool = tf.less(120.0, tf.sqrt(tf.add(tf.square(G[i, 3]),tf.square(G[i, 4]))))
        #    results = tf.cond(tf.less(120.0, tf.sqrt(tf.add(tf.square(G1[i, 3]),tf.square(G1[i, 4])))), lambda:tf.add(n_cross, add_penalty), lambda:n_cross)
        #D_fake, D_fake_logits, _ = tf.cond(results>loss_penalty, lambda:tuple(np.array(self.discriminator(G, is_training=True, reuse=True))/5.0),
        #lambda:self.discriminator(G, is_training=True, reuse=True))
    ...
    def output_csv(self, output_batch, label_num, data_name):
        ...
        with uproot3.recreate(data_name) as f:
            f["ttree"] = uproot3.newtree({"momentum": "float64",
                                         "phi": "float64",
                                         "theta": "float64",
                                         "x": "float64",
                                         "y": "float64",
            ...
            f["ttree"].extend({"momentum": np.array(simul_data)[:, 0],
                               "phi":np.array(simul_data)[:, 1],
                               "theta": np.array(simul_data)[:, 2],
                               "x": np.array(simul_data)[:, 3],
                               "y": np.array(simul_data)[:, 4],
                               "time": np.array(simul_data)[:, 5]})

    @property
    def model_dir(self):
        return "{}_{}_{}_{}_{}_{}_{}".format(
            self.model_name, self.dataset_name,
            self.z_dim,self.learning_rate,self.beta1,self.lambd,self.disc_iters)
