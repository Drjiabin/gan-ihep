...
class GAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        if dataset_name == 'mnist' or dataset_name == 'random':
            ...
            #data = np.array(load_rootracker(self.dataset_name))
            #print(data.shape)
            #self.data_X = np.reshape(data,(data.shape[0],1,9,1))
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            #net = lrelu(conv2d(x, 64, 1, 3, 2, 2, name='d_conv1'))
            ...
            #net = lrelu(bn(conv2d(net, 128, 1, 3, 2, 2, name='d_conv2'), is_training=is_training, scope='d_bn2'))
            #经过数组重构后，(64,7,7,128)-->(64,6272)
            #(64,1,3,128)-->(64,384)
            net = tf.reshape(net, [self.batch_size, -1])
            #经过线性处理后将矩阵，(64,6272)-->(64,1024)
            #(64,384)-->(64,384)
            net = lrelu(bn(linear(net, 384, scope='d_fc3'), is_training=is_training, scope='d_bn3'))
    ...
    def output_csv(self, epoch):
        ...
        for i in range(epoch):
            z_sample = np.random.uniform(-1, 1, size=(self.batch_size, self.z_dim))
            samples = self.sess.run(self.fake_images, feed_dict={self.z: z_sample})
            samples = np.reshape(samples,(64,784))
