...
class CGAN1(object):
    def __init__(self, sess, epoch, batch_size, z_dim, y_dim, dataset_name, checkpoint_dir, result_dir, log_dir, test_dir, input_height,input_width,output_height,output_width,
        ...
        self.model_name = "CGAN1"  # name for checkpoint
    ...
    def discriminator(self, x, y, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            net = lrelu(bn(conv2d(net, params_d[2][0], params_d[2][1], params_d[2][2], params_d[2][3], params_d[2][4], name=params_d[2][5]),is_training=is_training, scope='d_bn1'))
            net = lrelu(bn(conv2d(net, params_d[3][0], params_d[3][1], params_d[3][2], params_d[3][3], params_d[3][4], name=params_d[3][5]), is_training=is_training, scope='d_bn2'))
    ...
    def output_csv(self, output_batch):
        ...
        for i in range(output_batch):
            ...
            y_sample = np.zeros((self.batch_size, self.input_height, 1, 3))
            #11
            #y_sample[:,:,:,0] = y_sample[:,:,:,0]+1
            #13
            y_sample[:,:,:,1] = y_sample[:,:,:,1]+1
            ...
            simul_data.extend(samples)
        simul_data = pd.DataFrame(simul_data)
        simul_data.to_csv('cgan1_13.csv', index=False)
