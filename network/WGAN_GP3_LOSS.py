...
class WGAN_GP3_LOSS(object):
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, test_dir, input_height,input_width,output_height,output_width,
        ...
        self.model_name = "WGAN-GP3-LOSS"  # name for checkpoint
    ...
    def build_model(self):
        ...
        print(G)
        print("--------------")
        #G=(batch_size, input_height, input_width, 1)
        ...
        D_fake, D_fake_logits, _ = tf.cond(results>loss_penalty, lambda:tuple(np.array(self.discriminator(G, is_training=True, reuse=True))/10.0),
    ...
    def output_csv(self, output_batch):
        simul_data = []
        for i in range(output_batch):
            ...
            simul_data.extend(samples)
        simul_data = pd.DataFrame(simul_data)
        simul_data.to_csv('test_loss.csv', index=False)
