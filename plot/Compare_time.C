void Compare_time(){
  ...
  TString name ="time";
  double max = 36783502.590398408;
  double min = 13.929485411189741;
  //Number of bins
  int nbin = 200;

  //Time cut
  double timecutlow  = (TMath::Log(TMath::Log(TMath::Log(13.929485411189741))) - (-0.032))/1.082;
  double timecuthigh = (TMath::Log(TMath::Log(TMath::Log(36783502.590398408))) - (-0.032))/1.082;
  //cut1 mc4, resamp
  TCut cut1=Form("time>%f&&time<%f",13.929485411189741,36783502.590398408);
  //cut2 gan
  TCut cut2=Form("t>%f&&t<%f",timecutlow,timecuthigh);
  ...
  int n3=t_GAN->Draw("TMath::Exp(TMath::Exp(TMath::Exp((-0.032) + t*1.082)))>>"+h3name,cut2,"goff");
