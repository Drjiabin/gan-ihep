...
def load_comet(comet_data):
    ...
    for pdg_id in pdg_list:
        ...
        #one fifth
        data_tenpercent=roo_data.shape[0]//5
        roo_data = roo_data.iloc[0:data_tenpercent, :]
        #log transformation
        roo_data['t']=np.log(np.log(roo_data['t']))
        ...
        roodata_qt = quantile_transformer.fit_transform(roo_data)
        #roo_data[:, 1] = roodata_qt[:, 1]
        print("qt phi min is:",roo_data[:, 1].min())
        print("qt phi max is:",roo_data[:, 1].max())
        #roo_data[:, 1] = (roo_data[:, 1]-quantile_min)/(quantile_max-quantile_min)
        ...
        attribute_min_2212 = {'p':-1.088, 'p_phi':-3.142, 'p_theta':5.34e-05, 'x':-120, 'y':-120,'time':1.741}
        attribute_max_2212 = {'p':8.19, 'p_phi':3.142, 'p_theta':2.75, 'x':120, 'y':120,'time':2.22}
    ...
    trY = np.load('/hpcfs/bes/mlgpu/cuijb/gan/data/Geant4/eight_particles/label_2212_onetenth.npy')
    #get -11,11,13
    #trY = trY[0:71276135, :]
...
def get_feature_from_rootracker_file(pdg,file_name):
    ...
    events = uproot4.open(file_name)['RooTrackerTree']
