...
def load_roodata(file_num_start, file_num_end):
    ...
    for file_id in range(file_num_start, file_num_end):
        ...
        id_str = "%04d" % file_id
        file_name = os.path.join(path, "extracted_211_rootracker_"+id_str+".txt.root")
...
def main(argv):
    ...
    data_min = [-1.34, -3.142, 2e-05, -120, -120,-0.03]
    data_max = [7.76, 3.142, 2.189, 120, 120,0.53]

    with PdfPages('211.pdf') as pdf:
        path='/hpcfs/bes/mlgpu/cuijb/gan/generate_data'
        gan_data = load_gan_beam_data(path, '211.csv')
        input_height=512
        input_width=6
        ...
        list1 = [1,2,3,4,5,6]
        list_range=[[-1.34,7.76],[-3.15,3.15],[0, 2.2],[-120, 120],[-120, 120],[-0.03, 0.53]]
        #gan vs geant4 1d
        ...
        list4 = [0,1,2]
        range_2d=[[[-1.35,7.77,-3.15,3.15],[-1.35,7.77,0,2.2],[-3.15,3.15,0,2.2]],
                      [[-120,120,-120,120]],
                      [[-1.35,7.77,-120,120],[-1.35,7.77,-120,120]],
                      [[-3.15,3.15,-120,120],[-3.15,3.15,-120,120]],
                      [[0,2.2,-120,120],[0,2.2,-120,120]],
                      [[-0.03,0.54,-1.35,7.77],[-0.03,0.54,-3.15,3.15],[-0.03,0.54,0,2.2]],
