...
#    roo_path = os.path.join(path, file_name)
#    events = uproot.open(roo_path)['ttree']
#    df_all = events.pandas.df(['momentum','phi','theta','x','y','time'], flatten=True)
#    gen_data = np.array(df_all)
#    return gen_data
def load_gan_beam_data(path,file_name):
...
def load_roodata(file_num_start, file_num_end):
    ...
    for file_id in range(file_num_start, file_num_end):
        ...
        id_str = "%04d" % file_id
        file_name = os.path.join(path, "extracted_-11_rootracker_"+id_str+".txt.root")
...
def main(argv):
    ...
    data_min = [-3.08, -3.142, 7.48e-05, -120,-120,-0.032]
    data_max = [6.752, 3.142, 1.71, 120, 120,1.043]

    with PdfPages('minus_11.pdf') as pdf:
        path='/hpcfs/bes/mlgpu/cuijb/gan/generate_data'
        gan_data = load_gan_beam_data(path, 'minus_11.csv')
        ...
        #subtract_data = roo_data - gan_data
        ...
        list1 = [1,2,3,4,5,6]
        list_range=[[-3.08,6.752],[-3.142,3.142],[0,1.71],[-120, 120],[-120, 120],[-0.032, 1.05]]
        #gan vs geant4 1d
        ...
        for index,g4_key, gan_key in zip(list1,list_geant4, list_gan):
            plt.subplot(3, 3, index)
            my_plot_hist_log_compare(roo_data[:, index-1],gan_data[:, index-1],100,g4_key,'G4_'+g4_key,'GAN_'+gan_key,list_range[index-1])
        pdf.savefig(fig2)
        #plt.subplot(3, 3, 7)
        ...
        list4 = [0,1,2]
        range_2d=[[[-3.1,6.76,-3.15,3.15],[-3.1,6.76,7.48e-05,1.72],[-3.15,3.15,7.48e-05,1.72]],
                      [[-120,120,-120,120]],
                      [[-3.1,6.76,-120,120],[-3.1,6.76,-120,120]],
                      [[-3.15,3.15,-120,120],[-3.15,3.15,-120,120]],
                      [[7.48e-05,1.72,-120,120],[7.48e-05,1.72,-120,120]],
                      [[-0.033,1.05,-3.1,6.76],[-0.033,1.05,-3.15,3.15],[-0.033,1.05,7.48e-05,1.72]],
                      [[-0.033, 1.05, -120, 120],[-0.033, 1.05, -120, 120]]]
