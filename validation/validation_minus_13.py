...
def load_roodata(file_num_start, file_num_end):
    ...
    for file_id in range(file_num_start, file_num_end):
        ...
        id_str = "%04d" % file_id
        file_name = os.path.join(path, "extracted_-13_rootracker_"+id_str+".txt.root")
...
def main(argv):
    ...
    data_min = [-1.91, -3.142, 9.95e-05, -120, -120,-0.03]
    data_max = [7.1, 3.142, 2.35, 120, 120,0.6]

    with PdfPages('minus_13.pdf') as pdf:
        path='/hpcfs/bes/mlgpu/cuijb/gan/generate_data'
        gan_data = load_gan_beam_data(path, 'minus_13.csv')
        input_height=512
        input_width=6
        ...
        list1 = [1,2,3,4,5,6]
        list_range=[[-1.92,7.2],[-3.15,3.15],[9.96e-05,2.36],[-120, 120],[-120, 120],[-0.04, 0.61]]
        #gan vs geant4 1d
        ...
        list4 = [0,1,2]
        range_2d=[[[-1.92,7.2,-3.15,3.15],[-1.92,7.2,0,2.36],[-3.15,3.15,0,2.36]],
                      [[-120,120,-120,120]],
                      [[-1.92,7.2,-120,120],[-1.92,7.2,-120,120]],
                      [[-3.15,3.15,-120,120],[-3.15,3.15,-120,120]],
                      [[0,2.36,-120,120],[0,2.36,-120,120]],
                      [[-0.03,0.61,-1.92,7.2],[-0.03,0.61,-3.15,3.15],[-0.03,0.61,0,2.36]],
                      [[-0.03, 0.61, -120, 120],[-0.03, 0.61, -120, 120]]]
