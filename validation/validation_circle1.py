...
def extract_n_epoch(start_Epoch, end_Epoch, step,data1, data2):
    dic_GAN = {'px':0, 'py':0, 'pz':0,'global_x':0, 'global_y':0, 'time':0}
    dic_Geant4 = {'px':0, 'py':0, 'pz':0,'global_x':0, 'global_y':0, 'time':0}
...
def load_roodata(file_num_start, file_num_end):
    ...
    for file_id in range(file_num_start, file_num_end):
        ...
        #df_all = events.pandas.df(['beam_px','beam_py','beam_pz','global_x','beam_r', 'beam_z', "beam_t"], flatten=True)
        ...
        #z is a number
        df_all = df_all[(df_all['global_z']>4175.0027)and(df_all['global_z']<4175.003)and(np.sqrt(df_all['global_x']**2+df_all['global_y']**2)<120)]
        df_all = df_all.drop(['global_z'], axis=1)
...
def main(argv):
    ...
    #-------------------------------------------------------------------
    #extract GAN data
    GAN_data_path = "./results/1004-17-15-04"
    gan_data=load_gan_beam_data(GAN_data_path)

    #-------------------------------------------------------------------
    #
    parameter = read_yml('hyper_parameter', 'hyperparam51.yml')
    ...
    #circle
    data_min = {'px':-224.73, 'py':-247.78, 'pz':9.63e-08, 'global_x':-120, 'global_y':-120, 'time':13.94}
    data_max = {'px':150.62, 'py':196.54, 'pz':1136.383, 'global_x':120, 'global_y':120, 'time':33739774}
    start = 0
    ...
    with PdfPages(GAN_data_path+'/'+'train_results_visualise.pdf') as pdf:
        for iEpoch in range(start, end, step):
            ...
            list_gan = ['global_x','global_y','px','py','pz','p']
            list_geant4 = ['global_x','global_y','px','py','pz','p']
            list1 = [1,2,3,4,5,6,7]
            #gan vs geant4
            ...
            my_plot_hist_compare(time_arr_G4[np.where((time_arr_G4>0)and(time_arr_G4<5000))],
                                 time_arr_GAN[np.where((time_arr_GAN>0)and(time_arr_GAN<5000))],1000,'0<time<5000','G4_time','GAN_time')
            plt.subplot(3, 3, 8)
            my_plot_hist_compare(time_arr_G4[np.where((time_arr_G4>5000)and(time_arr_G4<1e7))],
                                 time_arr_GAN[np.where((time_arr_GAN>5000)and(time_arr_GAN<1e7))],1000,'5000<time<1e7','G4_time','GAN_time')
            fig1.suptitle("date:0815-19-21 train epoch is: %d" % epoch_num, fontsize=20)
            ...
            plt.subplot(3, 3, 7)
            my_plot_hist_log_compare(time_arr_G4[np.where((time_arr_G4>0)and(time_arr_G4<5000))],
                                 time_arr_GAN[np.where((time_arr_GAN>0)and(time_arr_GAN<5000))],1000,'0<time<5000','G4_time','GAN_time')
            plt.subplot(3, 3, 8)
            my_plot_hist_log_compare(time_arr_G4[np.where((time_arr_G4>5000)and(time_arr_G4<1e7))],
                                 time_arr_GAN[np.where((time_arr_GAN>5000)and(time_arr_GAN<1e7))],1000,'5000<time<1e7','G4_time','GAN_time')
            pdf.savefig(fig2)
            #
            #2d
            ...
            list2 = [['px','px','py'],['global_x'],['p','p'],['p','p','p'],['px','px'],['py','py'],['pz','pz'],['time','time']]
            list3 = [['py','pz','pz'],['global_y'],['global_x','global_y'],['px','py','pz'],['global_x','global_y'],['global_x','global_y'],
                     ['global_x','global_y'],['global_y', 'p']]
            list4 = [0,1,2]
            range_2d=[[[-150,100,-100,100],[-150,100,0,500],[-150,120,0,500]],
                      [[-125,125,-125,125]],
                      [[0,500,-125,125],[0,500,-125,125]],
                      [[0,500,-150,100],[0,500,-150,120],[0,500,0,500]],
                      [[-150,100,-120,120],[-150,100,-125,125]],
                      [[-150,120,-125,125],[-150,120,-125,125]],
                      [[0,500,-125,125],[0,500,-125,125]],
