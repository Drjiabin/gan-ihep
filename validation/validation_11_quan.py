...
def np2dict_sub(data):
    dic_sub = {'log(p)':0, 'p_phi':0, 'p_theta':0,'x':0, 'y':0, 'log(log(log(t)))':0}
    list1 = list(dic_sub.keys())
    for i, attribute in enumerate(list1):
        dic_sub[attribute]=data[:, i]
    return dic_sub
...
def main(argv):
    ...
    with PdfPages('cgan2_e.pdf') as pdf:
        path='/hpcfs/bes/mlgpu/cuijb/gan/generate_data'
        gan_data = load_gan_beam_data(path, 'cgan2_e.csv')
        input_height=512
        input_width=6
        roo_data = roo_data[0:gan_data.shape[0],:]

        #Antinormalization
        gan_data = Antinormalization(gan_data, 5.2,-5.2)
        #inverse quantile transform
        gan_data = quantile_transformer.inverse_transform(gan_data)
        ...
        for i in range(len(list2)):
            ...
            pdf.savefig(fig)
        #二维图的相减
        #sub_data = gan_data-roo_data
        #dict_sub = np2dict_sub(sub_data)
        #for i in range(len(list2)):
        #    fig = plt.figure(figsize=(27, 15))
        #    for j,k,l,m in zip(list1[0], list2[i], list3[i],list4):
        #       plt.subplot(2,3,j)
        #        my_plot_2d_hist(dict_sub[k], dict_sub[l], 30,k+' vs '+l)
        #    pdf.savefig(fig)
