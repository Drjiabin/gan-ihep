#lobal_x, global_y, global_z, px,py,pz,time
...
def read_yml(yml_type, yml_file):
    ...
    yml_path=os.path.join("/hpcfs/bes/mlgpu/cuijb/gan/yml/"+yml_type+"/batchsize",yml_file)
...
def main(argv):
    ...
    #-------------------------------------------------------------------
    #extract GAN data
    GAN_data_path = "./results/batchsize/0831-07-41"
    gan_data=load_gan_beam_data(GAN_data_path)

    #-------------------------------------------------------------------
    #
    parameter = read_yml('hyper_parameter', 'hyperparam15.yml')
    ...
    time1=time.time()
    epoch_num=1
    with PdfPages(GAN_data_path+'/'+'train_results_visualise_exp.pdf') as pdf:
        for iEpoch in range(start, end, step):
            ...
            #exp log p,t
            dic_GAN['p']=np.exp(dic_GAN['p'])
            dic_GAN['t']=np.exp(np.exp(np.exp(dic_GAN['t'])))
            ...
            list1 = [1,2,3,4,5,6]
            list_range=[[0,1151],[-3.1415926,3.1415926],[5.206e-05,3.14],[-120, 120],[-120, 120],[13, 1e7]]
            #gan vs geant4
            ...
            my_plot_hist_compare(p_data_G4[np.where((p_data_G4>0)&(p_data_G4<2))],
                                 p_data_GAN[np.where((p_data_GAN>0)&(p_data_GAN<2))],100,'0<p<2','G4_p','GAN_p')
            plt.subplot(1, 2, 2)
            my_plot_hist_compare(p_data_G4[np.where((p_data_G4>2)&(p_data_G4<500))],
                                 p_data_GAN[np.where((p_data_GAN>2)&(p_data_GAN<500))],100,'2<p<500','G4_p','GAN_p')
            pdf.savefig(fig3)
           ...
           #circle time,p log transformation
            range_2d=[[[0,1151,-3.14,3.14],[0,1151,0,3.14],[-3.14,3.14,0,3.14]],
                      [[-125,125,-125,125]],
                      [[0,1151,-125,125],[0,1151,-125,125]],
                      [[-3.14,3.14,-150,150],[-3.14,3.14,-150,150]],
                      [[0,3.14,-150,150],[0,3.14,-150,150]],
                      [[13,1e7,0,1151],[13,1e7,-3.14,3.14],[13,1e7,0,3.14]],
                      [[13, 1e7, -125, 125],[13, 1e7, -125, 125]]]
